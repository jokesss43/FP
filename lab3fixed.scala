...
object ShopSimulation {
  ...
  def showMenu(): (FiniteDuration, Int, Int, Int) = {
    ...
    val duration = readInt("Время работы магазина (секунды)", 10, 1, 300).seconds
    val cashiers = readInt("Количество кассиров", 2, 1, 20)
    val customers = readInt("Количество покупателей", 3, 1, 50)
    val queueSize = readInt("Максимальный размер очереди", 5, 1, 100)
